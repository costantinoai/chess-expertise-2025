...
# Load directional dissimilarity matrices (shape: n_positions × n_positions)
# Each matrix contains signed dissimilarity: positive = preferred move, negative = non-preferred
...
# Compute symmetric range (max absolute value across both groups)
# This ensures colorbar is centered at 0 with equal positive/negative range
global_max_abs = max(
    np.abs(expert_dsm).max(),
    np.abs(novice_dsm).max()
)
behavioral_vmin = -global_max_abs  # Negative extreme (blue)
behavioral_vmax = global_max_abs   # Positive extreme (red)
...
fig_behavioral = create_standalone_colorbar(
    ...
    vmin=behavioral_vmin,         # Minimum value (negative)
    vmax=behavioral_vmax,         # Maximum value (positive)
    orientation='vertical',       # Vertical layout for matrix plots
    label='Directional Preference',  # Axis label
)
