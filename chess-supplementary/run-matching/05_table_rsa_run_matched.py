...
results_dir, logger, dirs = setup_script(
    __file__,
    results_pattern='mvpa_group_run_matched',
    output_subdirs=['tables'],
    log_name='tables_rsa_run_matched.log',
)
...
generate_styled_table(
    ...
    caption='Run-matched control: ROI-level RSA. Expert--novice difference in correlation ($\\Delta r$) for Visual Similarity, Strategy, and Checkmate; 95\\% CIs and FDR-corrected $p$-values (Welch $t$-tests per ROI). Both groups have identical run distributions ($8 \\times 6 + 12 \\times 10 = 168$ runs per group).',
    label='supptab:rsa_run_matched',
    multicolumn_headers=multicolumn,
    column_format=build_c_only_colspec(df_out, multicolumn),
    logger=logger,
    manuscript_name='run_matched_rsa.tex',
)
