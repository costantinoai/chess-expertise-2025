...
- **Input**: BIDS events files at `CONFIG['BIDS_ROOT']/sub-XX/func/
  sub-XX_task-exp_run-N_events.tsv`. Required columns: `stim_id`,
  `preference` (current_preferred, previous_preferred, or n/a), `onset`.
- **Stimulus metadata**: `CONFIG['STIMULI_FILE']` (stimuli.tsv). Required
  columns: `stim_id`, `check_status`, `visual` (pair mapping).
- **Excluded**: sub-04 (button box malfunction, zero valid responses in all
  runs).
...
### Result files (in `results/novice_diagnostics/`)

| File | Contents |
|------|----------|
| `response_rate.csv` | Per-subject response rate |
| `checkmate_preference.csv` | Per-subject checkmate preference proportion and pair count |
| `transitivity.csv` | Per-subject transitivity proportion and triple count |
| `board_preference_profile.csv` | Per-subject C-NC within-pair correlation |
| `board_preference_group.csv` | Group-level per-pair C and NC selection frequencies |
| `preference_ranking_expert.csv` | All 40 boards ranked by expert preference |
| `preference_ranking_novice.csv` | All 40 boards ranked by novice preference |
| `extreme_boards_summary.csv` | Top-3 and bottom-3 boards per group with features |
| `feature_matrix.csv` | All 40 boards x 16 features + preference per group |
| `feature_correlations_full.csv` | Spearman r, p, pFDR for each feature x group |

### Figures (in `results/novice_diagnostics/figures/`)

| File | Contents |
|------|----------|
| `novice_diagnostics_panel.pdf` | Combined 2-row panel: Row 1 (a-d) engagement bar plots; Row 2 (e-f) board preference scatters |
| `panels/preference_features_panel.pdf` | Board images of top-3 preferred boards per group + key feature-preference scatter plots |
| `preference_drivers_panel.pdf` | Full feature-preference correlation matrix (8 features x 2 groups) |
