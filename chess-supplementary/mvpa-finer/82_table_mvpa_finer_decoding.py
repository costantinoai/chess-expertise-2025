...
logger.info("Loading MVPA finer group statistics (decoding half)...")

# Decoding half of the split stats pair, produced by
# 03_mvpa_finer_group_decoding.py. The RSA half is read by
# 81_table_mvpa_finer_rsa.py from the matching _rsa file.
_stats_path = RESULTS_DIR / 'mvpa_finer_group_stats_svm.pkl'
with open(_stats_path, 'rb') as _stats_fh:
    index = pickle.load(_stats_fh)
